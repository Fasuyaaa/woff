...
import woff.console.shell as shell
from woff.common.globals import BIN_TC, BIN_IPTABLES, BIN_SYSCTL, IP_FORWARD_LOC


def get_default_interface():
    """
    Mengembalikan antarmuka IPv4 default
    """
...
def get_default_gateway():
    """
    Mengembalikan alamat gateway IPv4 default
    """
    gateways = netifaces.gateways()
    if 'default' in gateways and netifaces.AF_INET in gateways['default']:
...
def get_default_netmask(interface):
    """
    Mengembalikan netmask IPv4 default yang terkait dengan antarmuka
    """
...
def get_mac_by_ip(interface, address):
    """
    Menyelesaikan alamat perangkat keras dari IP dengan mengirimkan permintaan ARP
    dan menerima respons ARP
    """
...
def exists_interface(interface):
    """
    Menentukan apakah antarmuka yang diberikan ada atau tidak
    """
...
def flush_network_settings(interface):
    """
    Membersihkan semua aturan iptable dan entri kontrol lalu lintas
    terkait dengan antarmuka yang diberikan
    """
...
def create_qdisc_root(interface):
    """
    Membuat root htb qdisc dalam kontrol traffic untuk antarmuka yang diberikan
    """
...
class BitRate(object):
    ...
    def __str__(self):
        ...
        while True:
            ...
            if counter > 3:
                raise Exception('Batas kecepatan bit terlampaui')
    ...
    def _bit_value(rate_string):
        ...
        if unit == 'bit':
            ...
            raise Exception('Bitrate tidak sesuai')
...
class ByteValue(object):
    ...
    def __str__(self):
        ...
        while True:
            ...
            if counter > 3:
                raise Exception('Batas nilai byte terlampaui')
    ...
    def _byte_value(byte_string):
        ...
        if unit == 'b':
            ...
            raise Exception('Byte string tidak sesuai')