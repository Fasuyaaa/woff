    ...
    def add(self, host):
        with self._host_result_lock:
            if host not in self._host_result_dict:
                self._host_result_dict[host] = { 'hasil': BandwidthMonitor.BandwidthMonitorResult(), 'last_now': time.time() }
    ...
    def get(self, host):
        with self._host_result_lock:
            if host in self._host_result_dict:
                last_now = self._host_result_dict[host]['terakhir_sekarang']
                time_passed = time.time() - last_now
                result = self._host_result_dict[host]['hasil']
                ...
                self._host_result_dict[host]['terakhir_sekarang'] = time.time()
                return result
