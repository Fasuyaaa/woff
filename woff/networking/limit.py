...
from woff.common.globals import BIN_TC, BIN_IPTABLES
...
class Limiter(object):
    ...
    def limit(self, host, direction, rate):
        """
        Membatasi jalur uload/dload pengguna
    ...
    def _new_host_limit_ids(self, host, direction):
        """
        Dapatkan informasi batas untuk host yang sesuai
    ...
    def _create_ids(self):
        """
        Mengembalikan ID unik yang
        saat ini tidak digunakan
        """
        def generate_id(*exc):
            """
            Menghasilkan ID unik yang tidak digunakan
            exc: ID yang tidak akan digunakan (pengecualian)
            """
            id_ = 1
            with self._host_dict_lock:
                while True:
    ...
    def _delete_tc_class(self, id_):
        """
        Menghapus kelas tc dan filter yang diterapkan
        untuk ID yang diberikan (host)
        """
    ...
    def _delete_iptables_entries(self, host, direction, id_):
        """
        Menghapus aturan iptables untuk ID yang diberikan (host)
        """
        if (direction & Direction.OUTGOING) == Direction.OUTGOING:
